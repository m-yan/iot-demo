...
import org.onem2m.cse.util.RoundRobinIterator;
...
public class MqttConnectionManager {
	...
	@Autowired
	private MessageProcessor notifyProcessor;

	public void connectAll() {
		String topic = prop.getTopicForSubscribe();
		...
		brokerURLs.forEach(url -> {
			...
			try {
				if (!mqttConnection.isConnected()) {
					...
					mqttConnection.subscribe(topic, notifyProcessor);
